...
async function startLogging() {
    taskName = await vscode.window.showInputBox({ prompt: "What task are you working on?" });
    if (!taskName || taskName.trim() === "") {
      vscode.window.showWarningMessage("Task logging canceled due to no valid task name provided.");
      return;
    }
    startTime = new Date();
    vscode.window.showInformationMessage(`Started logging time for task: ${taskName}`);
...
function logHours() {
    if (startTime && taskName) {
        ...
        try {
            let existingData = fs.existsSync(logFilePath) ? fs.readFileSync(logFilePath, 'utf-8') : "";
            let updated = false;
            let newData = existingData.split("\n").map(line => {
                if (line.includes(endTime.toLocaleDateString()) && line.includes(taskName)) {
            ...
            vscode.window.showInformationMessage(`Logged ${hours}h ${minutes}m ${seconds}s on ${taskName}. Details saved to worklog.txt`);
        } catch (err) {
            if (err instanceof Error) {
                vscode.window.showErrorMessage("Failed to log time: " + err.message);
            } else {
                vscode.window.showErrorMessage("Failed to log time due to an unknown error");
            }
        }
    } else {
        vscode.window.showInformationMessage("No active task to log.");
